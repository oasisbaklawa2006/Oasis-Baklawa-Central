...
| **E4** | Governance bar disabled actions | Engineering | **Yes** | Open inbox; inspect amber governance bar | `queue-disabled-governance-bar.png` | Reassign / Approve Draft / Send Automation locked | Click triggers action | **Blocked** (0 packets — RLS role_key) |
...
| **E6** | Override audit table count | Engineering / DBA | **Yes** | Run SQL on staging Supabase | [`audit-override-log-count.txt`](./audit-override-log-count.txt) | Zero inbox-driven rows in 24h (expected today) | Unexpected writes | **Complete** |
...
| **E11** | RBAC URL access note | Engineering | **No** | Static audit; staging test deferred | [`rbac-url-access.md`](./rbac-url-access.md) + [`rbac-url-access-staging.md`](./rbac-url-access-staging.md) | Documents gap; staging URL proof captured | Claims pass without proof | **Complete** (static + staging doc) |
| **E12** | Full inbox visibility | Engineering | **Yes** | Open inbox with insights column expanded | [`visibility-full-inbox.png`](./visibility-full-inbox.png) | Packets, thread, insights visible | Broken layout | **Partial** (shell only; empty list) |
...
| **E14** | POST_MERGE_PR69 smoke (14 checks) | Engineering | **Yes** | Follow `docs/POST_MERGE_PR69_OPERATOR_INBOX_SMOKE.md` | [`pr69-smoke-checklist.md`](./pr69-smoke-checklist.md) | All items checked | Any blocking failure | **Partial** (3 pass, 3 blocked, 8 deferred) |
...
| **E18** | Webhook auto-order env unset | Engineering | **Yes** | Supabase/Vercel env inspection | [`webhook-auto-order-env.txt`](./webhook-auto-order-env.txt) | `ENABLE_WA_WEBHOOK_AUTO_ORDER_WRITES` unset/false | Flag true in staging | **Complete** (conditional) |
...
| **E20** | Override + suggestion log SQL | Engineering / DBA | **Yes** | Same session as E6 | [`audit-suggestions-log-count.txt`](./audit-suggestions-log-count.txt) | Zero inbox-driven writes | Unexpected writes | **Complete** |
