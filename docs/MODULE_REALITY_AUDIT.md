...
## UI lockdown applied (2026-06-09)

**Branch:** `cursor/misleading-ui-lockdown-9b16`  
**Scope:** UI-only badges, warning copy, and disabled governed actions on preview/local/demo paths. No routes removed, no business logic or backend changes, no SQL/migrations.

### Shared components

| File | Lockdown |
|------|----------|
| `src/components/admin/OperationalStatusLabel.tsx` | Standard labels: LIVE, PREVIEW ONLY, LOCAL ONLY, NOT PERSISTED, DOES NOT CREATE SALES ORDER, DEMO DATA |
| `src/components/admin/GovernanceBoardLiveNotice.tsx` | PREVIEW ONLY + DEMO DATA banner on preview cards; LIVE badge on live reads; copy states write buttons disabled |
| `src/components/admin/DispatchReadinessEvidencePanel.tsx` | Evidence record buttons respect parent `canWrite` (blocked on preview/demo) |

### HIGH-risk screens — lockdown notes

| Module | Lockdown applied |
|--------|------------------|
| **WhatsApp — local draft panel** (`OperatorInboxDraftOrderPanel.tsx`) | LOCAL ONLY, NOT PERSISTED, DOES NOT CREATE SALES ORDER badges on extraction panel |
| **WhatsApp — local hint / AI preview** (`OperatorInboxReadOnlyPanels.tsx`) | Status badges on local parse and keyword preview panels |
| **WhatsApp — persisted draft** (`OperatorInboxSalesOrderDraftSection.tsx`) | LIVE when persisted; NOT PERSISTED before create; DOES NOT CREATE SALES ORDER always; “Approve for SO (staging only)” button label |
| **FinanceGovernanceBoard** | Preview card badges; governed write buttons disabled when `showPreviewCards` or non-Supabase `persistenceMode` |
| **DispatchReadinessBoard** | Preview card badges; readiness review + evidence writes disabled on preview/demo |
| **DispatchCompletionBoard** | Preview card badges; completion review/attest disabled on preview/demo |
| **DispatchFinalizationBoard** | Preview card badges; finalize/publish/reversal disabled on preview/demo |
| **StockFinalizationBoard** | PREVIEW ONLY / DEMO DATA / LIVE header badges; finalize consumption disabled on preview/demo/non-Supabase |
| **LabelCommandCenter** | DEMO DATA, PREVIEW ONLY, NOT PERSISTED header badges (demo payloads + localStorage prefs) |
| **InventoryCommandCenter** | PREVIEW ONLY, DEMO DATA badges + synthetic-feed warning |
| **InventoryRiskBoard** | PREVIEW ONLY, DEMO DATA badges |

### Labels used

- **LIVE** — governance boards with live read-model rows; persisted `sales_order_drafts` section when draft exists
- **PREVIEW ONLY** — `VITE_EXECUTION_PREVIEW_FALLBACK` sample cards; projection-only inventory surfaces; label JSON preview
- **LOCAL ONLY** — WhatsApp localStorage draft workflow and in-browser parses
- **NOT PERSISTED** — local draft extraction; label prefs; draft section before first persist
- **DOES NOT CREATE SALES ORDER** — local approve/reject; persisted draft approve path (staging stop at `APPROVED_FOR_SO`)
- **DEMO DATA** — governance preview samples; label demo payloads; inventory synthetic feeds; stock demo persistence

*Lockdown: frontend UI only. No SQL or migrations.*
