...
| **55** | Dispatch DPL | **Genuine implementation gap** (legacy route) closing in this lane; software DPL chain otherwise **complete with admissible evidence** | FACT-C1 carton RPCs; FACT-C2 `b2b_dispatch_packing_list_versions`; FACT-C3 `DispatchManagement` governed chain through `submit_b2b_dispatch_packing_list_to_finance` | FACT-E2E dispatch consignment/carton/scan/DPL/finance stages; `legacyDplMutationDecommission.test.ts`; source-truth registry DPL entry | `/admin/execution/dispatch` still mounted dead `operational_queue_items` board; stale nav/home links; physical UAT | **Central PR (this branch):** retire dispatch execution-board URL → `/admin/dispatch-mgmt` with contract test |
...
| Field | Value |
|---|---|
| **PR** | `cursor/lane-d-factory-points-51-55-closure-976a` |
| **Predecessor (must be merged first)** | FACT-E2E #433; FACT-C3 dispatch governed chain; Core FACT-C1/C2 migrations |
...
| Dispatch legacy execution URL | 55 | **Closing in this PR** |
...
**Lane D software closure verdict:** Points **51, 52, 53, 56** — no further independent Central implementation links at this head. Point **54** — upstream #410. Point **55** — one narrow legacy URL/nav correction (this PR). **No production mutations performed.**
