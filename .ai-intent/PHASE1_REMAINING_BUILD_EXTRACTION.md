...
| #210 | `fix(admin): redirect retired verification route` | `/admin/verification` no longer renders `VerificationWarRoom`; it is now a hard `<Navigate to="/admin/cmd-war-room" replace />`, matching the existing legacy-redirect pattern. This was this document's own Batch 1 (below), now complete. |
...
| Item | Route/component | Why it blocks Phase 1 | Evidence source | Exact recommended implementation | Files likely touched | Risk level | Suggested PR size | Acceptance criteria |
|---|---|---|---|---|---|---|---|---|
| ~~VerificationWarRoom hard redirect~~ | `/admin/verification` → `VerificationWarRoom.tsx` | **DONE — PR #210 (2026-07-07).** `/admin/verification` is now a hard `<Navigate to="/admin/cmd-war-room" replace />`. `VerificationWarRoom.tsx` was left in place (not deleted) per the PR's own scope. | PR #210 | — | — | — | — | Closed. |
| ~~Registry status accuracy for 3 static-preview screens~~ | `/admin/scan-timeline`, `/admin/inventory-risk-board`, `/admin/inventory-command-center` | **DONE — Post-Hygiene Status Alignment, 2026-07-07.** `SCREEN_REGISTRY.md` rows #93, #96, #97 changed from `BUILT_NEEDS_EVIDENCE` to `PARTIAL`, each with a note citing the static/hardcoded-input evidence. | `SCREEN_REGISTRY.md` Post-Hygiene Status Alignment section | — | — | — | — | Closed. |
...
| `/admin/verification` | Comment: "VerificationWarRoom is retired... kept active to avoid 404s." | N/A — route is now a hard redirect, not a rendered screen | Yes — PR #206 (badge) + PR #210 (hard redirect, complete) |
...
### Batch 1 — VerificationWarRoom hard redirect — **DONE (PR #210, 2026-07-07)**
...
### Batch 2 — Registry status correction for 3 static-preview screens — **DONE (Post-Hygiene Status Alignment, 2026-07-07)**
...
Batches 1 and 2 are now done (PR #210 and this Post-Hygiene Status Alignment pass, respectively — see Current Completed Hygiene PRs above). The recommended next PR is **Batch 3 — E2E evidence capture for the 5-screen governance-board family** (`/admin/dispatch-readiness`, `/admin/dispatch-completion`, `/admin/dispatch-finalization`, `/admin/stock-finalization`, `/admin/finance-governance`). It is read-only verification against code already confirmed solid (the shared `useGovernanceBoardState` live/preview pattern), carries no new risk, and is the highest-confidence path to legitimately promoting any of these five toward `BUILT_VALIDATED` per Quality Gates — rather than jumping straight into real backend wiring (Batches 4-6) before that evidence exists.

## Post-Hygiene Status Alignment - 2026-07-07

This document has been updated in place (not appended as a separate section) to reflect that Batches 1 and 2 of its own build sequence are complete, per PR #210 and the accompanying `SCREEN_REGISTRY.md`/`FEATURE_REGISTRY.md`/`PHASE1_ADMIN_CLOSEOUT_AUDIT_AND_BUILD_QUEUE.md` corrections made in the same pass. No feature is marked `BUILT_VALIDATED` as a result of this alignment — only route/registry hygiene closed out. Batch 3 (governance-board evidence capture) is now the live recommended next step.
