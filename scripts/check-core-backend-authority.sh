...
is_guard_artifact_path() {
  case "$1" in
    ...
    .github/workflows/core-backend-authority-advisory.yml|\
    .github/workflows/secret-exposure-guard.yml|\
    .github/workflows/secret-exposure-guard-advisory.yml|\
...
diff_base="$base_ref"
if merge_base="$(git merge-base "$base_ref" "$target_ref" 2>/dev/null)"; then
  diff_base="$merge_base"
fi
if ! git diff --name-status -z -M -C --find-copies-harder "$diff_base" "$target_ref" > "$commit_diff"; then
  fail "git diff failed for $diff_base..$target_ref"
fi
