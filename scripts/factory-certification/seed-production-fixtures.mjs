...
if (!baseUrl || !serviceRoleKey || !anonKey || credentialFile !== "/tmp/oasis-factory-certification.env") {
  throw new Error("FACTORY_CERT_SUPABASE_URL, FACTORY_CERT_LOCAL_SERVICE_ROLE_KEY, FACTORY_CERT_SUPABASE_ANON_KEY and the fixed certification credential file are required");
}
...
// Buyer checkout prerequisites mirror Core's own customer checkout contract.
...
// Create a disposable buyer identity as prerequisite data, then use its real
// authenticated session for add_customer_order_draft_line_v1 + submit_customer_order_v1.
...
const { error: buyerUserError } = await supabase.from("users").upsert({
  id: buyerId,
  email: buyerEmail,
  full_name: "Factory Cert Golden Buyer",
  role: "b2b_buyer",
...
const { data: checkoutRows, error: checkoutError } = await buyerClient.rpc("submit_customer_order_v1", {
  p_idempotency_key: "factory-cert-golden-order-checkout-v1",
...
if (!Array.isArray(checkoutRows) || checkoutRows.length !== 1 || !checkoutRows[0]?.order_id || !checkoutRows[0]?.order_number || checkoutRows[0]?.is_duplicate_submission) {
  throw new Error(`Golden-order governed Buyer checkout returned invalid facts: ${JSON.stringify(checkoutRows)}`);
}
const goldenOrderId = String(checkoutRows[0].order_id);
const { data: goldenItems, error: goldenItemsError } = await supabase
  .from("order_items")
  .select("id,order_id,product_id,quantity")
  .eq("order_id", goldenOrderId)
  .eq("product_id", GOLDEN_ORDER_FG_COMPONENT_PRODUCT_ID);
assertNoSupabaseError(goldenItemsError, "Golden-order item verification read failed");
if (!Array.isArray(goldenItems) || goldenItems.length !== 1 || Number(goldenItems[0].quantity) !== 5) {
  throw new Error(`Golden-order governed checkout did not create exactly one quantity-5 order item: ${JSON.stringify(goldenItems)}`);
}
const goldenOrderItemId = String(goldenItems[0].id);
await appendFile(credentialFile, `export FACTORY_CERT_GOLDEN_ORDER_ID='${goldenOrderId}'\nexport FACTORY_CERT_GOLDEN_ORDER_ITEM_ID='${goldenOrderItemId}'\n`, { encoding: "utf8" });
console.log(`Created governed golden Sales Order ${checkoutRows[0].order_number} through Buyer checkout.`);
