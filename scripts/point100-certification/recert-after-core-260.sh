...
# Final Point100 software recertification boundary after Core #290:
#   - Core main exact SHA 7b2a09d4a70ee9632c264b552c3265f2495ce48a
#   - protected Production Migration Release #182 / run 34661721779
...
export POINT100_CORE_VERIFIED_SHA="${POINT100_CORE_VERIFIED_SHA:-7b2a09d4a70ee9632c264b552c3265f2495ce48a}"
export POINT100_RECERT_AFTER_CORE_MIGRATION_RUN_ID="${POINT100_RECERT_AFTER_CORE_MIGRATION_RUN_ID:-34661721779}"
...
if [[ "${POINT100_CORE_VERIFIED_SHA}" != "7b2a09d4a70ee9632c264b552c3265f2495ce48a" ]]; then
  echo "Refusing Point100 final recert: unexpected Core SHA ${POINT100_CORE_VERIFIED_SHA}" >&2
  exit 1
fi

if [[ "${POINT100_RECERT_AFTER_CORE_MIGRATION_RUN_ID}" != "34661721779" ]]; then
  echo "Refusing Point100 final recert: unexpected production migration run ${POINT100_RECERT_AFTER_CORE_MIGRATION_RUN_ID}" >&2
  exit 1
fi

echo "==> Point100 final software recert after Core #290 / Production Migration Release #182"
