...
export default function AppverseMobileNav({ allowedModules }: AppverseMobileNavProps) {
  ...
  const visible = APPVERSE_WORKSPACES
    .filter((workspace) => canAccessWorkspace(workspace, allowedModules))
    .sort((a, b) => scoreWorkspace(a) - scoreWorkspace(b));

  const primary = visible.slice(0, 4);
  const governance = visible.find((workspace) => workspace.key === "governance");
  const items = governance && !primary.some((workspace) => workspace.key === governance.key)
    ? [...primary, governance]
    : visible.slice(0, 5);

  return (
    <nav
      aria-label="Mobile App-Verse workspaces"
      className="fixed inset-x-0 bottom-0 z-40 border-t border-border/80 bg-background/95 px-2 pb-[max(env(safe-area-inset-bottom),0.5rem)] pt-2 backdrop-blur lg:hidden"
    >
          ...
          return (
            <NavLink
              ...
              className={[
                ...
                isActive
                  ? "bg-[hsl(var(--appverse-premium))] text-[hsl(var(--appverse-olive))]"
                  : "text-muted-foreground hover:bg-[hsl(var(--appverse-utility))] hover:text-foreground",
              ].join(" ")}
            >
