...
import { Loader2, Phone, MapPin, StickyNote, Plus } from "lucide-react";
...
export default function ClientInteractionsTab({
  ...
  const fetchInteractions = async () => {
    ...
    const { data } = await supabase
      ...
      .select("id, company_id, interaction_type, notes, follow_up_date, created_at")
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <Plus size={18} className="text-[#C4A052]" /> Log Client Activity
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4 mt-2">
            ...
            <div>
              ...
              <Textarea value={formNotes} onChange={(e) => setFormNotes(e.target.value)} placeholder="Details of interaction..." />
            </div>
            <div>
