...
import { Package, ShoppingCart, AlertTriangle, CheckCircle2, Trash2, Loader2, Wand2, ChevronLeft, CreditCard, Banknote, Smartphone, X, Lock, Printer } from "lucide-react";
...
function groupByCartonType(items: any[]) {
  ...
  return Array.from(map.entries()).map(([cartonType, items]) => ({ cartonType, rule: getCartonRule(cartonType), items }));
}
...
const Cart = () => {
  ...
  const sortedItems = useMemo(() => [...items].sort((a, b) => (a.product?.name || "").localeCompare(b.product?.name || "")), [items]);
  ...
  // UPDATED: Sweets/Baklawa generally fall under the 5% GST Slab
  ...
  if (sortedItems.length === 0) return (
    <AppShell>
      <div className="px-5 pt-10 flex flex-col items-center justify-start min-h-[60vh] space-y-4">
        ...
        <button onClick={() => navigate("/catalogue")} className="text-[#B8860B] font-bold">Browse Catalogue</button>
      </div>
    </AppShell>
  );
      ...
      <div className="px-5 pt-8 space-y-6 pb-32 max-w-3xl mx-auto flex flex-col justify-start min-h-screen">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button onClick={() => navigate(-1)} className="p-2 -ml-2 hover:bg-slate-100 rounded-full"><ChevronLeft size={24} /></button>
            <h1 className="font-display text-2xl tracking-wide">Review Order</h1>
          </div>
          <button onClick={handlePrintSO} className="flex items-center gap-1.5 bg-slate-100 text-slate-700 px-3 py-2 rounded-xl text-xs font-bold hover:bg-slate-200 transition-colors">
        ...
        {sections.map((section) => {
          ...
          return (
            <motion.section key={section.cartonType} className={`bg-white rounded-[2rem] p-5 border shadow-sm ${isIncomplete ? "border-amber-200" : "border-slate-100"}`}>
               <h2 className="font-bold text-slate-800 text-sm mb-4">Category {section.cartonType} <span className="text-slate-400 font-medium text-xs">({section.rule.packsPerCarton} Packs = 1 Carton)</span></h2>
                  ...
                  return (
                    <div key={item.id} className="flex items-center justify-between">
                      <div className="flex-1 pr-4">
                        ...
                        <p className="text-[10px] text-slate-500 font-bold mt-0.5">Total: <span className="text-slate-800">{formatPrice(itemTotal)}</span></p>
                      </div>
                      <div className="flex items-center bg-slate-50 rounded-xl p-1 border border-slate-200">
                          <button onClick={() => updateQuantity(item.id, item.quantity - 1)} className="w-8 h-8 rounded-lg bg-white shadow-sm font-bold">{item.quantity === 1 ? <Trash2 size={12} className="text-red-500 mx-auto" /> : "−"}</button>
                          <span className="font-bold text-xs w-8 text-center">{item.quantity}</span>
                          <button onClick={() => updateQuantity(item.id, item.quantity + 1)} className="w-8 h-8 rounded-lg bg-slate-900 text-white shadow-sm font-bold">+</button>
                      </div>
                    </div>
                  );
                })}
               ...
               <div className="pt-4 mt-5 border-t border-slate-100">
                  {isIncomplete ? (
                    <div className="bg-amber-50 rounded-xl p-4 border border-amber-200 shadow-inner">
                       ...
                       <div className="h-1.5 w-full bg-amber-200/50 rounded-full overflow-hidden mb-4"><div className="h-full bg-amber-500" style={{ width: `${(remainder / section.rule.packsPerCarton) * 100}%` }} /></div>
                       ...
                       <div className="flex flex-wrap gap-2">
                         ...
                         <button onClick={() => handleAutoOptimize(section)} className="w-full mt-1 bg-[#B8860B] text-white py-2.5 rounded-lg text-xs font-bold flex justify-center items-center gap-2 shadow-sm hover:bg-[#9A7009]">
                    ...
                    <div className="bg-emerald-50 rounded-xl p-3 flex justify-center items-center gap-2 border border-emerald-100">
                      ...
                      <p className="text-[11px] font-bold text-emerald-800 uppercase tracking-wider">All Cartons Filled & Sealed</p>
                    </div>
                  )}
               </div>
            </motion.section>
        ...
        <motion.section className="bg-slate-900 text-white rounded-[2rem] shadow-2xl p-6 relative overflow-hidden print:hidden">
          <div className="absolute top-0 right-0 opacity-5 pointer-events-none"><Package size={120} className="-mt-4 -mr-4" /></div>
