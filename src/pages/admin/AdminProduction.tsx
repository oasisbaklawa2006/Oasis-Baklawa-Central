...
interface ProdOrder {
  ...
  company_id: string | null; created_at: string | null;
...
const AdminProduction = () => {
  ...
  const fetchOrders = async () => {
    ...
    const { data } = await supabase
      .from("orders")
      .select("id, status, sales_order_value, company_id, created_at, company:companies(business_name), order_items(id, quantity, product_id)")
  ...
  const filtered = orders.filter(o => tab === "production" ? o.status === "in_production" : o.status === "assembly");
  ...
  const handleAdvance = async (order: ProdOrder) => {
    ...
    await supabase.from("order_status_history").insert({ order_id: order.id, old_status: order.status, new_status: next });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-2">
        <button onClick={() => setTab("production")} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-ui-button transition-colors ${tab === "production" ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground hover:text-foreground"}`}>
          <Factory size={14} /> {t("In Production")} ({prodCount})
        </button>
        <button onClick={() => setTab("assembly")} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-ui-button transition-colors ${tab === "assembly" ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground hover:text-foreground"}`}>
      ...
      <div className="bg-card border border-border rounded-xl p-4" style={{ boxShadow: "0 2px 10px rgba(0,0,0,0.04)" }}>
        <div className="flex items-center justify-between mb-2">
          <span className="text-ui-label text-muted-foreground">{tab === "production" ? "Production" : "Assembly"} Workload</span>
          <span className="text-ui-kpi text-primary">{filtered.length} orders · {totalItems} packs</span>
        </div>
        <div className="h-2 bg-muted rounded-full overflow-hidden">
          <div className="h-full bg-primary/70 rounded-full transition-all" style={{ width: `${Math.min((filtered.length / Math.max(orders.length, 1)) * 100, 100)}%` }} />
        </div>
      </div>
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map(order => {
            ...
            const daysSince = order.created_at ? Math.floor((Date.now() - new Date(order.created_at).getTime()) / 86400000) : 0;
            const isDelayed = daysSince > 5;
            return (
              <div key={order.id} className={`bg-card border rounded-xl p-4 space-y-3 ${isDelayed ? "border-destructive/40" : "border-border"}`} style={{ boxShadow: "0 1px 6px rgba(0,0,0,0.04)" }}>
                <div className="flex justify-between items-start">
                  ...
                  {isDelayed && <span className="text-fine text-destructive font-semibold">Delayed {daysSince}d</span>}
                </div>
                ...
                <div className="h-1.5 bg-muted rounded-full overflow-hidden">
                  <div className="h-full bg-primary/60 rounded-full" style={{ width: `${Math.min((packs / maxItems) * 100, 100)}%` }} />
                </div>
                <button
