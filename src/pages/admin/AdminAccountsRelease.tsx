...
// --- FIXED: Added the missing STATUSES array ---
const STATUSES = [
  "draft",
  "submitted",
  "under_review",
...
const AdminAccountsRelease = () => {
  ...
  const fetchOrders = async () => {
    ...
    const { data, error } = await supabase
      ...
      .select(
        `
        id, status, sales_order_value, company_id, created_at, advance_required, advance_paid, payment_status,
    ...
    if (error) {
      ...
      // --- FIXED: Changed OrderCard to FinanceOrder to match your state interface ---
  ...
  const handleAction = async (order: FinanceOrder, action: PaymentAction) => {
    ...
    try {
      if (action === "request_advance") {
        ...
        await supabase
          .from("orders")
          .update({ advance_paid: advReq, status: "in_production", payment_status: "advance_paid" })
          .eq("id", order.id);
        await supabase
          .from("order_status_history")
          .insert({ order_id: order.id, old_status: order.status, new_status: "in_production" });
  ...
  const dispatchBlocked = orders.filter(
    (o) => ["ready_for_dispatch", "packing"].includes(o.status) && getReleaseStatus(o) === "finance_hold",
  ).length;

  const filtered =
    tab === "hold"
      ? orders.filter((o) => getReleaseStatus(o) === "finance_hold")
      : tab === "overdue"
        ? orders.filter((o) => getReleaseStatus(o) === "balance_pending")
        : orders;
  ...
  return (
    <div className="space-y-6">
      ...
      {filtered.length === 0 ? (
        ...
        <div
          className="rounded-xl overflow-visible border border-border bg-card"
                ...
                return (
                  <tr key={order.id} className="border-t border-border">
                    ...
                    <td className="px-4 py-3">
                      <span className={`px-2 py-1 rounded-full text-xs font-semibold ${rl.color}`}>{rl.label}</span>
                    </td>
                    <td className="px-4 py-3 text-right text-ui-cell text-muted-foreground">
                      {format(order.advance_required ?? 0)}
                    </td>
                    <td className="px-4 py-3 text-right text-ui-cell text-green-600">
                      {format(order.advance_paid ?? 0)}
                    </td>
                    <td className="px-4 py-3 text-right text-ui-cell text-foreground">
                      {format(order.sales_order_value ?? 0)}
                    </td>
                    ...
                    <td className="px-4 py-3 text-right">
                      {actions.length > 0 ? (
                        ...
                        <span className="text-ui-label text-muted-foreground flex items-center gap-1 justify-end">
                          <Check size={14} /> Paid
                        </span>
                      )}
                    </td>
