...
const OperationsController = () => {
  ...
  const myDepartmentProducts = departmentProducts.filter(
    (p) => p.category === myDepartment || (!p.category && myDepartment === "Baklawa"),
  );
          ...
          {myDepartmentOrders.length === 0 ? (
            ...
            myDepartmentOrders.map((order) => {
              const deptItems = order.items.filter(
                (item) => item.category === myDepartment || (!item.category && myDepartment === "Baklawa"),
              );
                ...
                >
                  <div className="flex justify-between items-start">
                    <div>
                      <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1 flex items-center gap-1">
                        <Hash size={10} /> Order Fulfillment
                      </p>
                  ...
                  <div className="mt-4 flex items-center justify-between border-t border-slate-100 pt-3">
                    <span className="text-sm font-bold text-[#B8860B] flex items-center gap-1.5">
                      <Package size={16} /> {deptItems.length} {myDepartment} Items req.
                    </span>
        ...
        <div className="p-4">
          <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-4 mb-4">
            ...
            <p className="text-[10px] text-emerald-600 font-bold uppercase tracking-wider mt-1">
              Record items produced outside of specific orders. Sent directly to Store Inventory.
            </p>
          </div>
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      {qty > 0 && <p className="text-[10px] font-black text-emerald-600 uppercase">Logging...</p>}
                    </div>
                  </div>

                  <div className="flex items-center bg-slate-50 rounded-lg border border-slate-200 h-9">
                    <button
                      onClick={() => adjustStoreLog(product.id, -10)}
                      className="w-8 h-full flex items-center justify-center text-slate-500 font-black active:bg-slate-200 rounded-l-lg"
                    >
                      -
                    </button>
                    ...
                    <button
                      onClick={() => adjustStoreLog(product.id, 10)}
                      className="w-8 h-full flex items-center justify-center text-slate-700 font-black active:bg-slate-200 rounded-r-lg"
                    >
                      +
                    </button>
                  </div>
                        ...
                        <div className="flex-1 flex flex-col justify-between">
                          <div>
                            ...
                            <div className="flex items-center gap-2 mt-1">
                              ...
                              {isExcess && (
                                <span className="text-[10px] bg-purple-100 text-purple-700 px-1.5 py-0.5 rounded font-black uppercase">
                          ...
                          <div className="flex items-center gap-3 mt-3 bg-slate-50 p-1.5 rounded-xl border border-slate-200 w-full max-w-[240px]">
                            ...
                            <div className="flex-1 text-center">
                              <span
                                className={`text-2xl font-black ${isExcess ? "text-purple-600" : isComplete ? "text-emerald-600" : "text-slate-900"}`}
                              >
                                {produced}
                              </span>
                            </div>
              ...
              return (
                <div className="p-4 bg-white border-t border-slate-200 pb-safe shadow-[0_-10px_40px_rgba(0,0,0,0.05)]">
                  <div className="flex items-center gap-2 mb-3 px-1 text-emerald-600">
                    <CheckCircle2 size={16} /> <p className="text-xs font-bold uppercase tracking-wider">Target Met</p>
                  </div>
                  <button
                    onClick={handleProcessOrder}
                    disabled={isSubmitting}
                  ...
                  >
                    {isSubmitting ? <Loader2 className="animate-spin" /> : <span>Complete Tasks & Sync</span>}
                  </button>
                </div>
              );
            })()}
