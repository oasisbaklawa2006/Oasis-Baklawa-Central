...
import { 
  ...
  ShieldCheck, AlertTriangle, X, FileText, Send, Clock, CheckCheck
} from "lucide-react";
import TopNavBar from "@/components/TopNavBar";

// ─── Types ───
// Updated to perfectly match your Supabase database columns
interface Application {
  id: string;
  business_name: string;
  trade_name?: string | null;
...
const ClientsGovernance = () => {
  ...
  const fetchApplications = async () => {
    ...
    // The 'as unknown as Application[]' tells TypeScript to trust us on the format
  ...
  const handleApprove = async (app: Application) => {
    ...
    await supabase.from("audit_logs").insert({
      ...
      entity_name: app.business_name, entity_id: app.id
    });
  ...
  const handleReject = async (app: Application) => {
    ...
    if (!error) {
      await supabase.from("audit_logs").insert({
        ...
        entity_name: app.business_name, entity_id: app.id, reason: rejectionReason
      });
      await fetchApplications();
      setSelectedApp(null);
    } else {
  ...
  const handleSendInvite = async (app: Application) => {
    ...
    setInviteSending(app.id);
    const { error } = await supabase.from("portal_access_invites").insert({
      application_id: app.id, invite_email: app.contact_email, status: "pending", sent_at: new Date().toISOString()
    });
  ...
  if (loading) {
    return <div className="min-h-screen flex justify-center items-center bg-background"><Loader2 className="animate-spin text-primary" size={32} /></div>;
  }
            ...
            {filteredApps.length === 0 ? (
              ...
              filteredApps.map((app) => (
                ...
                >
                  ...
                  <div className="col-span-3">
                    <p className="text-sm text-foreground">{app.contact_person || 'N/A'}</p>
                    <p className="text-xs text-muted-foreground mt-0.5">{app.mobile_number || 'N/A'}</p>
                  </div>
                  <div className="col-span-3">
                    <p className="text-sm text-foreground">
        ...
        {selectedApp && (
          <>
            ...
            >
              <div className="flex items-center justify-between p-6 border-b border-border bg-muted/10">
                <h2 className="text-lg font-display tracking-wide text-foreground">{selectedApp.business_name}</h2>
                <button onClick={() => setSelectedApp(null)} className="p-2 text-muted-foreground hover:text-foreground rounded-full hover:bg-muted transition-colors">
                  <X size={20} />
                </button>
              </div>
              ...
              <div className="p-6 flex-1 overflow-y-auto space-y-6">
                ...
                {/* Company Info */}
                <div>
                  <h3 className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-2">Company Information</h3>
                  <div className="bg-card p-4 rounded-xl border border-border space-y-3">
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <p className="text-[10px] text-muted-foreground uppercase">GST Number</p>
                    ...
                    <div>
                      <p className="text-[10px] text-muted-foreground uppercase">Registered Address</p>
                      <p className="text-sm font-medium text-foreground">{selectedApp.registered_address || 'N/A'}</p>
                    </div>
                  </div>
                </div>
                ...
                {/* Contact Info */}
                <div>
                  <h3 className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-2">Contact Details</h3>
                  <div className="bg-card p-4 rounded-xl border border-border space-y-3">
                    <p className="text-sm font-medium flex items-center gap-2"><User size={14} className="text-muted-foreground"/> {selectedApp.contact_person || 'N/A'}</p>
                    <p className="text-sm font-medium flex items-center gap-2"><Phone size={14} className="text-muted-foreground"/> {selectedApp.mobile_number || 'N/A'}</p>
                    <p className="text-sm font-medium flex items-center gap-2"><Mail size={14} className="text-muted-foreground"/> {selectedApp.contact_email || 'N/A'}</p>
                  </div>
                </div>
                ...
                {selectedApp.status === 'pending' && (
                  <div>
                    <h3 className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-2">Governance Action</h3>
                    <div className="bg-muted/10 p-4 rounded-xl border border-border space-y-4">
                      <div>
                        <label className="text-[10px] font-bold text-muted-foreground uppercase mb-1 block">Assign Pricing Slab</label>
                        <select 
                          value={selectedSlab}
                          onChange={(e) => setSelectedSlab(e.target.value)}
                          className="w-full text-sm p-2.5 bg-background border border-border rounded-lg outline-none focus:border-primary"
                      ...
                      <div>
                        <label className="text-[10px] font-bold text-muted-foreground uppercase mb-1 block">Admin Notes (Optional)</label>
                        <input 
                          type="text"
                          value={adminNotes}
                          onChange={(e) => setAdminNotes(e.target.value)}
                ...
                {selectedApp.status === 'approved' && (
                  <div>
                    <h3 className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-2">Portal Access</h3>
                    <div className="bg-muted/10 p-4 rounded-xl border border-border space-y-4">
                      {getInviteForApp(selectedApp.id) ? (
                        <div className="flex items-center gap-2 text-sm font-medium text-emerald-600">
                          <CheckCheck size={16} /> Invite Sent / Active
                      ...
                      <button 
                        ...
                        className="w-full flex items-center justify-center gap-2 py-2.5 rounded-lg border border-primary/30 text-primary hover:bg-primary/5 transition-colors text-sm font-bold"
                      >
                        {inviteSending === selectedApp.id ? <Loader2 size={16} className="animate-spin" /> : <Send size={16} />}
                        {getInviteForApp(selectedApp.id) ? "Access Granted" : "Send Portal Invite"}
                      </button>
                    </div>
                  </div>
