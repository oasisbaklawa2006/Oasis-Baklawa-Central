...
const DUMMY_APPS: Application[] = [
...

  {
    ...
    user_id: null,
    admin_notes: "Premium client, priority processing",
  ...
  {
    id: "d4",
    business_name: "Southern Spice Traders",
    trade_name: null,
    business_type: "Distributor",

    contact_person: "Karthik Nair",
    contact_email: "karthik@sst.in",
    mobile_number: "+91 77654 32108",

    gst_number: "32AABCU9606R1ZP",
    expected_volume: "100-500 kg",
    city: "Kochi",
    state: "Kerala",

    registered_address: "23, Spice Market Road, Mattancherry, Kochi 682002",
    status: "approved",
    ...
    rejection_reason: null,
    assigned_price_tier: "Slab B",
  },

  {
    id: "d5",
    business_name: "Himalayan Treats",
    trade_name: "HT",
    business_type: "Retailer",

    contact_person: "Sonam Dorji",
    contact_email: "sonam@ht.in",
    mobile_number: "+91 66543 21098",

    gst_number: null,
    expected_volume: "50-100 kg",
    city: "Shimla",
    state: "Himachal Pradesh",

    registered_address: "5, Mall Road, Shimla 171001",
    status: "rejected",
    ...
    rejection_reason: "Missing GST registration",
    assigned_price_tier: null,
  },
];
...
const AdminClients = () => {
  ...
  const handleSendInvite = async (app: Application) => {
    ...
    if (existing && existing.status === "pending") {
      toast.error("A pending invite already exists");
      return;
    }
                    ...
                    >
                      ...
                      <SelectContent>
                        {pricingSlabs.map((slab) => (
                          <SelectItem key={slab.id} value={slab.slab_name}>
                            {slab.slab_name}
                          </SelectItem>
                        ))}
                      </SelectContent>
...
const PortalInviteSection = ({
  ...
  onSendInvite: (a: Application) => void;
  sending: boolean;
}) => (
    ...
    {invite ? (
      <div className="bg-muted/30 rounded-lg p-3 space-y-1.5">
        ...
        <p className="text-xs text-muted-foreground">
          Sent: {invite.sent_at ? new Date(invite.sent_at).toLocaleDateString() : "—"}
        </p>

        {invite.accepted_at && (
          <p className="text-xs text-muted-foreground">Accepted: {new Date(invite.accepted_at).toLocaleDateString()}</p>
        )}
      </div>
    ) : (
      <div className="bg-red-50 rounded-lg p-3">
