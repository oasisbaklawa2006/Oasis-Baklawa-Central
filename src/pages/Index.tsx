...
const SmartReorder = () => {
      ...
      try {
        ...
        if (data && data.order_items) {
          const formattedItems = data.order_items
            .filter((item: any) => item.product_id) // Ensure product exists
            .map((item: any) => ({
  ...
  const handleBulkAddToCart = async () => {
    ...
    setIsAdding(true);
    try {
      // Push all items to the user's cart safely
      ...
      navigate("/cart"); // Instantly take them to the cart to checkout properly
    } catch (error) {
  ...
  // If they have no past orders, show a sleek placeholder instead of a broken box
  if (items.length === 0) {
    return (
      <motion.div
