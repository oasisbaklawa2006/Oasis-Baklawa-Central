...
const getMinPacks = (cartonType: string | null) => {
  if (cartonType === "C") return 3;
  return 1;
};

const getCartonSize = (cartonType: string | null) => {
  if (cartonType === "C") return 9;
  return 12;
};
...
const ProductDetail = () => {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ data, error }) => {
        if (!error && data) {
          setProduct(data);
          setQty(getMinPacks(data.carton_type));
        }
        setLoading(false);
      });
  }, [id]);
    ...
    return (
      <AppShell>
        <div className="flex flex-col items-center justify-center min-h-[60vh] gap-3">
          ...
          <button onClick={() => navigate("/catalogue")} className="text-primary font-body font-semibold hover:underline">
            Back to Catalogue
          </button>
        </div>
      </AppShell>
  ...
  const adjustQty = (delta: number) => {
    setQty((prev) => {
      ...
      if (next < minPacks) return minPacks;
      if (next > cartonSize * 3) return prev;
      return next;
    });
  };
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="px-5 py-6 space-y-6">
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="space-y-1">
            <h1 className="font-display text-2xl tracking-wide text-foreground">{product.name}</h1>
            {isAuthenticated ? (
              <>
                <p className="font-fine text-lg text-foreground font-bold">
                  {formatPrice(product.price_per_kg)}{" "}
                  <span className="text-sm font-normal text-muted-foreground">per kg + taxes</span>
                </p>
                <p className="font-fine text-xs text-muted-foreground">Pack Size: {product.pack_size || "1kg"}</p>
              </>
            ) : (
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
            <Tabs defaultValue="description" className="w-full">
              <TabsList className="w-full bg-muted/60 rounded-xl p-1">
                <TabsTrigger value="description" className="flex-1 rounded-lg font-body text-xs data-[state=active]:bg-card data-[state=active]:shadow-card">Description</TabsTrigger>
                <TabsTrigger value="logistics" className="flex-1 rounded-lg font-body text-xs data-[state=active]:bg-card data-[state=active]:shadow-card">Logistics</TabsTrigger>
              </TabsList>

              <TabsContent value="description" className="mt-4">
                <div className="bg-card rounded-2xl shadow-card p-5">
                  <p className="font-body text-sm text-foreground leading-relaxed">{product.description || "No description available."}</p>
                </div>
              </TabsContent>
          ...
          {isAuthenticated ? (
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-center min-w-[60px]">
                    ...
                    <p className="font-body text-[11px] text-muted-foreground">× {product.pack_size || "1kg"} Packs</p>
                  </div>
                  <button onClick={() => adjustQty(1)} className="w-10 h-10 rounded-xl bg-muted border border-border flex items-center justify-center hover:border-primary/50 transition-colors">
            ...
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="bg-card rounded-2xl shadow-card p-5 space-y-3 text-center">
              ...
              <p className="font-body text-sm text-muted-foreground">Trade Members Only — Login to view pricing</p>
              <button
