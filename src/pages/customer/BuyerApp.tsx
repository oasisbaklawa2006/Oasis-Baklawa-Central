...
function ProductCard({ product, price, onAdd, onBuy, onView }: { product: Product; price?: BuyerPrice; onAdd: (id: string, quantity: number) => void; onBuy: (id: string, quantity: number) => void; onView: (id: string) => void }) {
  const minimum = Math.max(Number(price?.minimum_order_quantity || 1), 1);
  const increment = Math.max(Number(price?.order_increment || 1), 1);
      ...
      {price ? (
        <>
          ...
          <div className="mt-auto flex items-center gap-2 pt-4">
            <button type="button" aria-label={`Decrease ${productTitle(product)} quantity`} onClick={() => { setQuantity(Math.max(minimum, quantity - increment)); }} className="rounded-lg border p-2"><Minus size={14} aria-hidden /></button>
            <span className="min-w-10 text-center text-sm" aria-live="polite">{quantity}</span>
            <button type="button" aria-label={`Increase ${productTitle(product)} quantity`} onClick={() => { setQuantity(quantity + increment); }} className="rounded-lg border p-2"><Plus size={14} aria-hidden /></button>
            <button type="button" onClick={() => { onAdd(product.id, quantity); }} className="ml-auto rounded-lg border px-3 py-2 text-xs font-bold">Add</button>
            <button type="button" onClick={() => { onBuy(product.id, quantity); }} className="rounded-lg bg-primary px-3 py-2 text-xs font-bold text-primary-foreground">Buy</button>
          </div>
        </>
...
function Catalogue({ data }: { data: ReturnType<typeof useBuyerData> }) {
  ...
  return (
    <section className="space-y-5" aria-labelledby="catalogue-heading">
      ...
      {visible.length === 0 ? <Empty title="No products found" text="Try another search or check back after catalogue updates." action={query || category !== "all" ? <button type="button" onClick={() => { setQuery(""); setCategory("all"); }} className="rounded-lg border px-4 py-2 text-sm font-semibold">Clear filters</button> : undefined} /> : (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{visible.map((product) => <ProductCard key={product.id} product={product} price={priceById.get(product.id)} onAdd={(id, quantity) => { void add(id, quantity); }} onBuy={(id, quantity) => { void buy(id, quantity); }} onView={(id) => { navigate(`/buyer/catalogue/${id}`); }} />)}</div>
      )}
    </section>
  );
}
          ...
          {price ? (
            <>
              ...
              <div className="flex items-center gap-3">
                <button type="button" aria-label="Decrease quantity" onClick={() => { setQuantity(Math.max(minimum, quantity - increment)); }} className="rounded-lg border p-3"><Minus size={16} aria-hidden /></button>
                <span className="w-12 text-center" aria-live="polite">{quantity}</span>
                <button type="button" aria-label="Increase quantity" onClick={() => { setQuantity(quantity + increment); }} className="rounded-lg border p-3"><Plus size={16} aria-hidden /></button>
              </div>
              <div className="grid gap-3 sm:grid-cols-2"><button type="button" onClick={() => { void add(false); }} className="min-h-12 rounded-xl border px-4 py-3 font-bold">Add to cart</button><button type="button" onClick={() => { void add(true); }} className="min-h-12 rounded-xl bg-primary py-3 font-bold text-primary-foreground">Buy now</button></div>
            </>
...
function Cart({ data }: { data: ReturnType<typeof useBuyerData> }) {
  ...
  const draftMeta = data.draft[0];
  const readinessMessages = customerReadinessMessages(draftMeta?.readiness_issues as Json, undefined, undefined);
  const canSubmit = lines.length > 0 && (!draftMeta || draftMeta.readiness_status === "ready");
  ...
  const submit = async () => {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : "Order submission failed. You can safely retry.");
    } finally {
      setSubmitting(false);
    }
  };
...
function Documents({ data }: { data: ReturnType<typeof useBuyerData> }) {
  return <section className="space-y-5" aria-labelledby="documents-heading"><div><p className="text-sm text-muted-foreground">Customer documents</p><h1 id="documents-heading" className="font-display text-3xl font-semibold">Documents</h1></div><div className="rounded-2xl border border-dashed bg-card p-8 text-center"><FileText size={30} className="mx-auto text-muted-foreground" aria-hidden /><h2 className="mt-3 font-semibold">Documents appear when issued</h2><p className="mt-2 text-sm text-muted-foreground">No customer-facing proforma invoice or final invoice is available yet. We will show Core-issued documents here without creating local numbers or files.</p><Link to={data.orders.length > 0 ? "/buyer/orders" : "/buyer/catalogue"} className="mt-5 inline-flex min-h-11 items-center rounded-xl bg-primary px-4 py-3 text-sm font-semibold text-primary-foreground">{data.orders.length > 0 ? "View orders" : "Browse catalogue"}</Link></div></section>;
}
...
function Empty({ title: heading, text, action, icon }: { title: string; text: string; action?: ReactNode; icon?: ReactNode }) {
  return <div className="rounded-2xl border border-dashed p-10 text-center">{icon || <CheckCircle2 size={28} className="mx-auto text-muted-foreground" aria-hidden />}<p className="mt-3 font-semibold">{heading}</p><p className="mt-1 text-sm text-muted-foreground">{text}</p>{action && <div className="mt-4">{action}</div>}</div>;
}
