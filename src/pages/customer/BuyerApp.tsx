  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const [priceRows, productRows, draftRows, companyRows, orderRows, itemRows, ticketRows] = await Promise.all([
        ...
        supabase.from("products").select("id,name,sku,description,image_url,category,is_active,visible_in_catalog").eq("is_active", true).eq("visible_in_catalog", true),
...
/** Renders the mobile-first Buyer navigation without writing application state. */
function BuyerNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-20 border-t bg-background/95 backdrop-blur sm:sticky sm:top-0 sm:border-b sm:border-t-0">
      <div className="mx-auto flex max-w-6xl justify-around px-2 py-2 sm:justify-start sm:gap-8 sm:px-6">
        {links.map(([href, label, Icon]) => (
          <button
            key={href}
            onClick={() => { navigate(href); }}
            className={`flex min-w-16 flex-col items-center gap-1 text-[11px] sm:flex-row sm:text-sm ${location.pathname === href ? "text-primary" : "text-muted-foreground"}`}
          >
            <Icon size={18} />
            <span>{label}</span>
          </button>
    ...
    <article className="rounded-2xl border bg-card p-4 shadow-sm">
      <button onClick={() => { onView(product.id); }} className="block w-full text-left">
        <div className="mb-3 aspect-[4/3] overflow-hidden rounded-xl bg-muted">
          {product.image_url ? <img src={product.image_url} alt={title(product)} className="h-full w-full object-cover" /> : <div className="flex h-full items-center justify-center text-3xl">🍪</div>}
        </div>
        <p className="font-semibold">{title(product)}</p>
      </button>
      <p className="text-xs text-muted-foreground">{product.sku || "SKU pending"} · {price?.uom || "unit"}</p>
      {price ? (
        <>
          <p className="mt-2 text-lg font-bold text-primary">{money(price.selling_price)}</p>
          <p className="text-[11px] text-muted-foreground">MOQ {minimum} {price.minimum_order_uom || price.uom || "units"}</p>
          <div className="mt-3 flex items-center gap-2">
            <button aria-label="Decrease quantity" onClick={() => { setQuantity(Math.max(minimum, quantity - increment)); }} className="rounded-lg border p-2"><Minus size={14} /></button>
            <span className="min-w-10 text-center text-sm">{quantity}</span>
            <button aria-label="Increase quantity" onClick={() => { setQuantity(quantity + increment); }} className="rounded-lg border p-2"><Plus size={14} /></button>
            <button onClick={() => { onAdd(product.id, quantity); }} className="ml-auto rounded-lg border px-3 py-2 text-xs font-bold">Add</button>
            <button onClick={() => { onBuy(product.id, quantity); }} className="rounded-lg bg-primary px-3 py-2 text-xs font-bold text-primary-foreground">Buy</button>
          </div>
        </>
...
/** Provides searchable, category-filtered catalogue data and governed cart entry points. */
function Catalogue({ data }: { data: ReturnType<typeof useBuyerData> }) {
  ...
  return (
    <section className="space-y-5">
      <div><p className="text-sm text-muted-foreground">Shop approved B2B products</p><h1 className="text-2xl font-bold">Catalogue</h1></div>
      <div className="flex flex-col gap-3 sm:flex-row">
        <label className="flex flex-1 items-center gap-2 rounded-xl border bg-card px-3"><Search size={18} className="text-muted-foreground" /><input value={query} onChange={(e) => { setQuery(e.target.value); }} placeholder="Search products or SKU" className="w-full bg-transparent py-3 text-sm outline-none" /></label>
        <select value={category} onChange={(e) => { setCategory(e.target.value); }} className="rounded-xl border bg-card px-3 py-3 text-sm">{categories.map((c) => <option key={c} value={c}>{c === "all" ? "All categories" : c}</option>)}</select>
      </div>
      {visible.length === 0 ? <Empty title="No products found" text="Try another search or check back after catalogue updates." /> : (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {visible.map((p) => <ProductCard key={p.id} product={p} price={priceById.get(p.id)} onAdd={(id, quantity) => { void add(id, quantity); }} onBuy={(id, quantity) => { void buy(id, quantity); }} onView={(id) => { navigate(`/buyer/catalogue/${id}`); }} />)}
        </div>
      )}
    </section>
  );
...
function ProductDetail({ data, productId }: { data: ReturnType<typeof useBuyerData>; productId: string }) {
  ...
  const add = async () => { try { await customerAppClient.addLine(productId, quantity); await data.refresh(); toast.success("Added to your cart"); navigate("/buyer/cart"); } catch (e) { toast.error(e instanceof Error ? e.message : "Unable to add item"); } };
  return (
    <section className="space-y-5">
      <button onClick={() => { navigate("/buyer/catalogue"); }} className="inline-flex items-center gap-2 text-sm text-muted-foreground"><ArrowLeft size={16} /> Back to catalogue</button>
      <div className="grid gap-6 md:grid-cols-2">
        <div className="aspect-square overflow-hidden rounded-3xl bg-muted">{product.image_url ? <img src={product.image_url} alt={title(product)} className="h-full w-full object-cover" /> : <div className="flex h-full items-center justify-center text-6xl">🍪</div>}</div>
        <div className="space-y-4">
          <p className="text-sm text-muted-foreground">{product.category || "Approved catalogue"}</p>
          <h1 className="text-3xl font-bold">{title(product)}</h1>
          ...
          {price ? (
            <>
              ...
              <p className="text-sm">MOQ {price.minimum_order_quantity || 1} {price.minimum_order_uom || price.uom || "units"}</p>
              <div className="flex items-center gap-3">
                <button onClick={() => { setQuantity(Math.max(Number(price.minimum_order_quantity || 1), quantity - Number(price.order_increment || 1))); }} className="rounded-lg border p-2"><Minus size={16} /></button>
                <span className="w-12 text-center">{quantity}</span>
                <button onClick={() => { setQuantity(quantity + Number(price.order_increment || 1)); }} className="rounded-lg border p-2"><Plus size={16} /></button>
              </div>
              <button onClick={() => { void add(); }} className="w-full rounded-xl bg-primary py-3 font-bold text-primary-foreground">Add to cart</button>
            </>
...
function Cart({ data }: { data: ReturnType<typeof useBuyerData> }) {
  ...
  const lines = data.draft.filter(
    (line): line is BuyerDraftLine & { line_id: string; product_id: string } => Boolean(line.line_id && line.product_id),
  );
  ...
  return (
    <section className="space-y-5">
      ...
      {lines.length === 0 ? <Empty title="Your cart is empty" text="Browse the catalogue to add approved products." action={<button onClick={() => { navigate("/buyer/catalogue"); }} className="rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground">Browse catalogue</button>} /> : (
        <>
          <div className="space-y-3">
            {lines.map((line) => {
              ...
              const increment = Number(price?.order_increment || 1);
              return (
                <div key={line.line_id} className="flex items-center gap-3 rounded-xl border bg-card p-3">
                  <div className="flex-1"><p className="font-semibold">{line.product_name_snapshot || title(p)}</p><p className="text-xs text-muted-foreground">{money(line.unit_price_snapshot || price?.selling_price)} · {line.uom_snapshot || price?.uom || "unit"}</p></div>
                  <button onClick={() => { void mutate(() => customerAppClient.updateLine(line.line_id, Math.max(Number(price?.minimum_order_quantity || 1), Number(line.quantity || 1) - increment))); }} className="rounded-lg border p-2"><Minus size={14} /></button>
                  <span className="w-8 text-center text-sm">{line.quantity}</span>
                  <button onClick={() => { void mutate(() => customerAppClient.updateLine(line.line_id, Number(line.quantity || 1) + increment)); }} className="rounded-lg border p-2"><Plus size={14} /></button>
                  <button onClick={() => { void mutate(() => customerAppClient.removeLine(line.line_id)); }} className="rounded-lg p-2 text-destructive"><Trash2 size={16} /></button>
                </div>
              );
            })}
          </div>
...
function OrderDetail({ data, orderId }: { data: ReturnType<typeof useBuyerData>; orderId: string }) {
  ...
  const lines = data.items.filter((i) => i.order_id === orderId);
...
function Support({ data }: { data: ReturnType<typeof useBuyerData> }) {
  ...
  const submit = async () => { if (!orderId || !description.trim()) return; setSending(true); try { await customerAppClient.submitTicket(orderId, issue, description.trim()); toast.success("Support ticket submitted"); setDescription(""); await data.refresh(); } catch (e) { toast.error(e instanceof Error ? e.message : "Unable to submit ticket"); } finally { setSending(false); } };
  return <section className="space-y-5"><div><p className="text-sm text-muted-foreground">We are here to help</p><h1 className="text-2xl font-bold">Support</h1></div><div className="rounded-2xl border bg-card p-5 space-y-3"><select value={orderId} onChange={(e) => { setOrderId(e.target.value); }} className="w-full rounded-xl border bg-background px-3 py-3 text-sm"><option value="">Select an order</option>{data.orders.map((o) => <option key={o.order_id} value={o.order_id}>{o.order_number}</option>)}</select><select value={issue} onChange={(e) => { setIssue(e.target.value); }} className="w-full rounded-xl border bg-background px-3 py-3 text-sm"><option>Damaged Goods</option><option>Missing Items</option><option>Wrong Shipment</option><option>Other</option></select><textarea value={description} onChange={(e) => { setDescription(e.target.value); }} placeholder="Describe the issue" rows={4} className="w-full rounded-xl border bg-background px-3 py-3 text-sm" /><button disabled={sending || !orderId || !description.trim()} onClick={() => { void submit(); }} className="rounded-xl bg-primary px-4 py-3 text-sm font-bold text-primary-foreground disabled:opacity-50">{sending ? "Submitting…" : "Submit ticket"}</button></div><div className="space-y-2">{data.tickets.map((t) => <div key={t.ticket_id} className="rounded-xl border bg-card p-4 text-sm"><div className="flex justify-between"><strong>{t.issue_type}</strong><span className="text-muted-foreground">{t.customer_status}</span></div><p className="mt-1 text-muted-foreground">{t.description}</p></div>)}</div></section>;
}

/** Presents the customer-safe Buyer home summary and navigation shortcuts. */
function Home({ data }: { data: ReturnType<typeof useBuyerData> }) {
  const navigate = useNavigate();
  return <section className="space-y-5"><SystemAlertMarquee /><div className="rounded-3xl bg-primary p-6 text-primary-foreground"><p className="text-sm opacity-80">Welcome back</p><h1 className="mt-1 text-3xl font-bold">{data.company?.business_name || "Oasis buyer"}</h1><p className="mt-3 max-w-md text-sm opacity-90">Browse approved products, keep your carton rules intact, and submit a governed order in a few taps.</p><button onClick={() => { navigate("/buyer/catalogue"); }} className="mt-5 rounded-xl bg-background px-4 py-3 text-sm font-bold text-foreground">Browse catalogue</button></div><div className="grid gap-3 sm:grid-cols-3"><button onClick={() => { navigate("/buyer/cart"); }} className="rounded-2xl border bg-card p-4 text-left"><ShoppingBag className="text-primary" /><p className="mt-3 font-semibold">Cart</p><p className="text-xs text-muted-foreground">{data.draft.filter((l) => l.line_id).length} lines ready</p></button><button onClick={() => { navigate("/buyer/orders"); }} className="rounded-2xl border bg-card p-4 text-left"><Package className="text-primary" /><p className="mt-3 font-semibold">Orders</p><p className="text-xs text-muted-foreground">{data.orders.length} submitted</p></button><button onClick={() => { navigate("/buyer/support"); }} className="rounded-2xl border bg-card p-4 text-left"><LifeBuoy className="text-primary" /><p className="mt-3 font-semibold">Support</p><p className="text-xs text-muted-foreground">{data.tickets.length} tickets</p></button></div></section>;
}
...
export function BuyerAccessRequest() {
  ...
  const fields = [
    { key: "businessName", label: "Business name", required: true, value: form.businessName, setValue: (value: string) => { setForm((current) => ({ ...current, businessName: value })); } },
    { key: "contactName", label: "Contact name", required: true, value: form.contactName, setValue: (value: string) => { setForm((current) => ({ ...current, contactName: value })); } },
    { key: "contactEmail", label: "Work email", required: true, value: form.contactEmail, setValue: (value: string) => { setForm((current) => ({ ...current, contactEmail: value })); } },
    { key: "contactPhone", label: "Phone number", required: true, value: form.contactPhone, setValue: (value: string) => { setForm((current) => ({ ...current, contactPhone: value })); } },
    { key: "gstNumber", label: "GST number (optional)", required: false, value: form.gstNumber, setValue: (value: string) => { setForm((current) => ({ ...current, gstNumber: value })); } },
    { key: "address", label: "Registered address (optional)", required: false, value: form.address, setValue: (value: string) => { setForm((current) => ({ ...current, address: value })); } },
  ] as const;
  return <main className="mx-auto min-h-screen max-w-lg px-6 py-10"><p className="text-sm text-muted-foreground">Oasis Baklawa B2B</p><h1 className="mt-1 text-3xl font-bold">Request Buyer access</h1><p className="mt-2 text-sm text-muted-foreground">Tell us about your company. Approval is required before catalogue pricing and checkout are available.</p><div className="mt-6 space-y-3">{fields.map((field) => <label key={field.key} className="block text-sm font-medium">{field.label}<input required={field.required} value={field.value} onChange={(e) => { field.setValue(e.target.value); }} className="mt-1 w-full rounded-xl border bg-card px-3 py-3" /></label>)}<button disabled={submitting} onClick={() => { void submit(); }} className="w-full rounded-xl bg-primary py-3 font-bold text-primary-foreground disabled:opacity-50">{submitting ? "Submitting…" : "Submit access request"}</button></div></main>;
}
