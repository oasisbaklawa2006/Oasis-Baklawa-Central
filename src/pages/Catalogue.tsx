...
import { useCart } from "@/hooks/useCart"; // <-- Switched to the new DB Cart!
...
// Extracted Product Card to handle its own local Quantity Stepper
const ProductCard = ({ product }: { product: any }) => {
  ...
  const clientDiscountRate = 0.15; // MOCK: Premium Distributor 15% Slab
  ...
  return (
    ...
    >
      ...
      <div className="w-full aspect-square overflow-hidden bg-muted p-4 flex items-center justify-center">
        {product.image_url ? (
          <img src={product.image_url} alt={product.name} className="w-full h-full object-contain group-hover:scale-105 transition-transform duration-500 drop-shadow-md" />
        ) : (
      ...
      <div className="p-4 flex flex-col flex-1">
        <p className="text-ui-h5 text-foreground leading-tight group-hover:text-[#B8860B] transition-colors line-clamp-2">{product.name}</p>
        ...
        {isAuthenticated ? (
          <div className="mt-2 space-y-1 flex-1">
            ...
            <div className="pt-1 flex items-end gap-2">
              ...
              <p className="text-[10px] text-slate-400 line-through leading-none mb-0.5">{formatPrice(baseWholesalePrice)}</p>
            </div>
            
            {/* The Quick Add Stepper Engine */}
            <div className="pt-3 mt-auto" onClick={(e) => e.stopPropagation()}>
              <div className="flex items-center gap-2">
                <div className="flex items-center bg-slate-50 rounded-xl p-1 border border-slate-200 h-10 flex-1">
                  <button onClick={() => setBoxes(b => Math.max(1, b - 1))} className="w-8 h-full rounded-lg bg-white shadow-sm font-bold text-slate-700 active:scale-95">−</button>
                  <span className="font-bold text-sm flex-1 text-center text-slate-900">{boxes}</span>
                  <button onClick={() => setBoxes(b => b + 1)} className="w-8 h-full rounded-lg bg-slate-900 text-white shadow-sm font-bold active:scale-95">+</button>
                </div>
                <button onClick={handleAdd} disabled={isAdding} className="w-10 h-10 rounded-xl bg-[#B8860B] text-white flex items-center justify-center shadow-md active:scale-95 hover:bg-[#9A7009] transition-colors flex-shrink-0 disabled:opacity-50">
          ...
          <div className="mt-2 space-y-2 flex-1 flex flex-col justify-end">
            ...
            <button onClick={(e) => { e.stopPropagation(); navigate("/login"); }} className="w-full py-2 rounded-xl bg-muted text-foreground text-ui-button hover:bg-muted/80 transition-colors">
              Login / Apply
            </button>
          </div>
        )}
...
const Catalogue = () => {
  ...
  // Pulling the LIVE DB Cart Data for the floating bar
  const { items, loading: cartLoading } = useCart();
  const { isAuthenticated } = useAuth();

  // Floating Cart Math (5% GST)
  ...
  return (
    <AppShell>
      <div className="px-5 py-6 space-y-8 pb-32">
        ...
        {/* Header */}
        <motion.h1 initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="text-display-h1 text-foreground">
          Product Catalogue
        </motion.h1>
            ...
            {subCategories.map((sub) => (
              <button
                ...
                className={`flex-shrink-0 px-5 py-2 rounded-full text-ui-button transition-all ${
                  activeSub === sub ? "bg-primary text-primary-foreground shadow-fab" : "bg-card text-foreground shadow-card hover:shadow-fab"
                }`}
              >
                {sub}
              </button>
