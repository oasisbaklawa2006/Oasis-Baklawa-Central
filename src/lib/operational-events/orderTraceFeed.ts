...
export function buildOrderOperationalFeedFromTrace(input: {
  ...
  totalOrdered: number;
  totalPacked: number;
  /** Optional WA correlation on the order row — does not load packet messages here */
  linkedWamid?: string | null;
  ...
  const wamid = input.linkedWamid?.trim();
  if (wamid) {
    out.push({
      id: `oe:${orderId}:whatsapp.order_correlation_hint`,
      kind: OperationalEventKind.WHATSAPP_ORDER_CORRELATION,
      category: "communication",
      severity: "info",
      title: "WhatsApp correlation on order",
      detail:
        "This order row references a WhatsApp id. Match in Operator Inbox by thread context — packet messages are not loaded here.",
      occurredAt: null,
      sortKey: 160,
      actor: { role: "system", displayLabel: "Linkage hint" },
      entities: orderEntity(orderId),
      source: SOURCE,
    });
  }
