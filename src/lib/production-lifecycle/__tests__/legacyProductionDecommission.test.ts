...
describe("Point88 legacy production decommission", () => {
  it("AdminInventory no longer inserts into daily_production_logs", () => {
    const src = readFileSync(join(SRC_ROOT, "pages/admin/AdminInventory.tsx"), "utf8");
  ...
  it("PHH lifecycle mutations route through productionGovernedRpc", () => {
    const intake = readFileSync(join(SRC_ROOT, "components/phh/JobIntakeTab.tsx"), "utf8");
    const execution = readFileSync(join(SRC_ROOT, "components/phh/JobExecutionTab.tsx"), "utf8");
    const quickEntry = readFileSync(join(SRC_ROOT, "components/phh/QuickEntryTab.tsx"), "utf8");
    const dayEnd = readFileSync(join(SRC_ROOT, "components/phh/DayEndSignoffTab.tsx"), "utf8");
  ...
  it("FactoryTVModule remains read-only with no governed RPC imports", () => {
    const tv = readFileSync(join(SRC_ROOT, "components/FactoryTVModule.tsx"), "utf8");
  ...
  it("allocation surfaces use productionGovernedRpc for shortage demand", () => {
    const rgs = readFileSync(join(SRC_ROOT, "pages/admin/ReadyGoodsStore.tsx"), "utf8");
    const planner = readFileSync(join(SRC_ROOT, "pages/admin/RgsProductionDemandPlanner.tsx"), "utf8");
