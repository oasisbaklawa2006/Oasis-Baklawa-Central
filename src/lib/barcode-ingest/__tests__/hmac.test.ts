...
describe("hmac", () => {
  ...
  it("verifies valid signature", async () => {
    ...
    const result = await verifyHmacSignature({ body, idempotencyKey, signatureHeader: signature, secret });
  ...
  it("rejects missing signature", async () => {
    const result = await verifyHmacSignature({ body, idempotencyKey, signatureHeader: "", secret });
  ...
  it("falls back to alias secret", () => {
    expect(resolveSigningSecret({ CENTRAL_SCAN_SIGNING_SECRET: "alias-only" })).toBe("alias-only");
  });
