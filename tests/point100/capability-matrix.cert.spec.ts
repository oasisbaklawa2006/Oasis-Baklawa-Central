 ...
 * full operational lifecycle. Never silently skips: missing env causes an
 * explicit certification failure rather than a green skipped test.
 */
...
test("POINT100 :: generate capability/blocker matrix from executable probes", async ({ page }) => {
  ...
  const ledger = writeDressRehearsalLedger(
    {
      schema_version: 1,
      harness: "point100-capability-matrix",
