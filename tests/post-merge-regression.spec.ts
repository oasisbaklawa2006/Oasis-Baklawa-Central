/**
 * Post-merge production regression (buyer → finance credit → ops → RBAC).
 * Run: TEST_PREVIEW_URL=https://cursor-central-vercel.vercel.app … npx playwright test tests/post-merge-regression.spec.ts --project=desktop-chrome-size
 */
...
async function dumpCatalogueDebug(page: Page, testSlug: string) {
  console.log("[post-merge catalogue debug] URL:", page.url());
  const headings = await page.locator("h1, h2, h3").allTextContents();
  console.log("[post-merge catalogue debug] visible headings:", JSON.stringify(headings, null, 0));
  const shot = `test-results/post-merge-catalogue-debug-${testSlug}.png`;
  await page.screenshot({ path: shot, fullPage: true }).catch(() => {});
  console.log("[post-merge catalogue debug] screenshot:", shot);
}
...
async function drillCatalogueToFirstAddToCart(page: Page, testSlug: string) {
  ...
  const bulkTile = page.getByRole("button").filter({ has: page.getByText("Bulk Sweets & Nuts", { exact: true }) });
...
async function addToCartFromCatalogueWithRetry(page: Page, testSlug: string) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    await page.screenshot({ path: `test-results/post-merge-add-attempt-${i}-${testSlug}.png`, fullPage: true }).catch(() => {});
    console.log(`[post-merge] Add to Cart attempt ${i + 1}/${n} result: ${result}`);
    await page.waitForTimeout(600);
  }

  console.log("[post-merge add failure] console:", diag.consoleLines.join("\n") || "(none)");
  console.log("[post-merge add failure] network:", diag.networkLines.join("\n") || "(none)");
...
async function assertUnauthenticatedBlockedFromAdminRoute(page: Page, path: string) {
  await page.goto(`${PREVIEW_URL}${path}`, { waitUntil: "domcontentloaded", timeout: 60000 });
  await page.waitForLoadState("networkidle", { timeout: 30000 }).catch(() => {});
  const url = page.url();
  expect(/\/login/i.test(url)).toBeTruthy();
}

test.describe("Post-merge production regression", () => {
  ...
  const OPS_EMAIL = process.env.TEST_OPS_EMAIL || "ops@oasisbaklawa.com";
  const OPS_PASSWORD = process.env.TEST_OPS_PASSWORD || "production_manager";

  let orderPrefix: string | null = null;

  test("Buyer: catalogue → cart → SO → receipt → order survives refresh", async ({ page }) => {
    await login(page, BUYER_EMAIL, BUYER_PASSWORD);

    await drillCatalogueToFirstAddToCart(page, "regression");
    await addToCartFromCatalogueWithRetry(page, "regression");
    ...
    orderPrefix = m?.[1]?.toUpperCase() ?? null;
    ...
    await expect(page.getByText(new RegExp(`Order #${orderPrefix}`, "i")).first()).toBeVisible({ timeout: 60000 });
  });

  test("Finance: board loads, receipt visible, approve credit, push to floor, in production", async ({ browser }) => {
    expect(orderPrefix).toBeTruthy();
    const prefix = orderPrefix!;
    ...
    const receiptLink = fp.getByRole("link", { name: /Open full receipt/i });
    await expect(receiptLink).toBeVisible({ timeout: 30000 });
  ...
  test("Operations: login, /admin/operations, SO and line fields when orders present", async ({ browser }) => {
    expect(orderPrefix).toBeTruthy();
    const prefix = orderPrefix!;
    ...
    const ourOrderMarker = op.locator("p").filter({ hasText: new RegExp(`#${prefix}`, "i") });
    const emptyState = op.getByText(/No active operations/i);

    await expect(ourOrderMarker.or(emptyState).first()).toBeVisible({ timeout: 120000 });

    const isEmpty = await emptyState.isVisible().catch(() => false);
    if (!isEmpty) {
      await expect(ourOrderMarker.first()).toBeVisible({ timeout: 15000 });
      await expect(op.getByText(/Qty:/i).first()).toBeVisible({ timeout: 15000 });
      await expect(op.getByText(/Pack:/i).first()).toBeVisible({ timeout: 15000 });
      await expect(op.getByText(/Carton:/i).first()).toBeVisible({ timeout: 15000 });
    }
  ...
  test("RBAC: unauthenticated /admin/finance-board → login", async ({ page }) => {
    await assertUnauthenticatedBlockedFromAdminRoute(page, "/admin/finance-board");
  });

  test("RBAC: unauthenticated /admin/operations → login", async ({ page }) => {
    await assertUnauthenticatedBlockedFromAdminRoute(page, "/admin/operations");
  });
});
