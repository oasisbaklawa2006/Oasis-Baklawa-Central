...
test.describe("Post-fix #483 buyer approval re-test (UAT-0018 + UAT-0020)", () => {
  ...
  test.afterAll(() => {
    ...
    writeTrancheIndex(INDEX_PATH, "post-fix-483", `UAT-0018 + UAT-0020 (current main @ ${POST_FIX_483_BASELINE_SHA.slice(0, 8)})`, rows, [
      "**Pre-fix evidence preserved** in `tranche-02/` — not overwritten.",
      `**Post-fix deploy SHA:** \`${POST_FIX_483_BASELINE_SHA}\` (current main resolved at run time)`,
      ...
      closedSet.length > 0 ? `**CLOSED post-fix:** ${closedSet.join(", ")}` : `**No FAIL-IDs closed yet** — run with secrets on current-main deploy @ ${POST_FIX_483_BASELINE_SHA.slice(0, 8)}.`,
    ...
    appendFailureLedger(FAILURE_PATH, `post-fix-483 (current main @ ${POST_FIX_483_BASELINE_SHA.slice(0, 8)})`, allFailures, []);
  });
});
