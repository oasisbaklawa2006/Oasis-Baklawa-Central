...
async function verifyAccessToken(accessToken: string): Promise<{
  ...
  try {
    ...
    // Never log provider payload or credentials; only outcome metadata.
...
async function resolveCanonicalAuthUser(e164: string, normalized: string): Promise<{
  ...
  // No governed public identity owns the phone. Create exactly one new pending user.
  // If an orphan Auth identity already owns the phone, Auth uniqueness must reject
  // creation; we fail closed instead of guessing which orphan is authoritative.
  const email = internalEmailFor(normalized);
  ...
  if (pendingError) {
    // Do not mint a usable session if the governed profile could not be established.
    // The Auth row is intentionally left for reconciliation rather than silently
    // presenting an ungoverned identity as successful login.
    throw new Error("pending_profile_create_failed");
  }
...
serve(async (req) => {
  ...
  try {
    ...
    if (body.mode === "login_otp") {
      // Unused legacy mode previously returned the generated OTP to the caller.
      // It is intentionally disabled; MSG91 Widget + server verification is the
      // sole mobile-login authority.
    ...
    if (body.mode === "verify_widget") {
      ...
      // Browser phone is corroboration only. It never establishes identity.
      ...
      try {
        ...
        const conflict = [
          ...
          "phone_linked_to_missing_auth_identity",
