-- Issue #232 forward repair for nullable governed-state classification.
-- Null accountability state must never suppress pending, closure, or illegal-disposition exceptions.
...
begin
  ...
  with classified as (
    ...
    select
      ...
      count(*) filter (where is_governed_accounted is true)::bigint as governed_accounted,
      count(*) filter (
        where effective_disposition = 'ACTIVE_PENDING'
          and is_governed_accounted is not true
...
begin
  ...
  with classified as (
    ...
    select
      ...
      array_remove(array[
        case
          when c.is_governed_accounted is not true
            and (
...
comment on function public.get_whatsapp_authorized_channel_accountability_reconciliation_summary() is
  'Read-only Issue #232 reconciliation summary. Null or unsupported accountability state fails closed and cannot suppress pending, closure, or illegal-disposition accountability.';

comment on function public.get_whatsapp_authorized_channel_accountability_reconciliation_exceptions(interval, integer) is
  'Read-only Issue #232 exception register. Null or unsupported accountability state fails closed and is classified from disposition, ownership, next-action, and closure evidence.';
