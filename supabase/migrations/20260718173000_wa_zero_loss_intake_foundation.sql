...
create table if not exists public.whatsapp_business_intakes (
  ...
  constraint whatsapp_business_intakes_accounting_match check (
    reconciliation_status = 'ACCOUNTED'
  )
);

create unique index if not exists whatsapp_business_intakes_packet_unique
  on public.whatsapp_business_intakes(packet_id)
  where packet_id is not null;

create unique index if not exists whatsapp_business_intakes_provider_message_unique
  on public.whatsapp_business_intakes(provider_message_id)
  where provider_message_id is not null;

create index if not exists whatsapp_business_intakes_active_queue_idx
  on public.whatsapp_business_intakes(disposition, lifecycle_state, sla_due_at);

create index if not exists whatsapp_business_intakes_owner_idx
...
create table if not exists public.whatsapp_business_intake_audit_log (
  id uuid primary key default gen_random_uuid(),
...
create index if not exists whatsapp_business_intake_audit_log_intake_idx
...
drop policy if exists whatsapp_business_intakes_team_select on public.whatsapp_business_intakes;
create policy whatsapp_business_intakes_team_select
on public.whatsapp_business_intakes
for select
to authenticated
using (public.is_team_member(auth.uid()));

drop policy if exists whatsapp_business_intakes_team_insert on public.whatsapp_business_intakes;
create policy whatsapp_business_intakes_team_insert
on public.whatsapp_business_intakes
for insert
to authenticated
with check (public.is_team_member(auth.uid()));

drop policy if exists whatsapp_business_intakes_team_update on public.whatsapp_business_intakes;
create policy whatsapp_business_intakes_team_update
on public.whatsapp_business_intakes
for update
to authenticated
using (public.is_team_member(auth.uid()))
...
drop policy if exists whatsapp_business_intake_audit_team_select on public.whatsapp_business_intake_audit_log;
create policy whatsapp_business_intake_audit_team_select
on public.whatsapp_business_intake_audit_log
for select
to authenticated
using (public.is_team_member(auth.uid()));

drop policy if exists whatsapp_business_intake_audit_team_insert on public.whatsapp_business_intake_audit_log;
create policy whatsapp_business_intake_audit_team_insert
on public.whatsapp_business_intake_audit_log
for insert
to authenticated
with check (public.is_team_member(auth.uid()));
...
select
  ...
  (
    ...
    - count(*) filter (where disposition = 'ACTIVE_PENDING')
    - count(*) filter (where disposition = 'EXPLICITLY_CLOSED')
  )::bigint as unaccounted_potential_orders
...
comment on table public.whatsapp_business_intakes is
  'Authoritative B2B WhatsApp intake ledger. Every order-like or unresolved-risk intake must remain converted, actively pending, or explicitly closed.';
comment on view public.whatsapp_business_intake_reconciliation is
  'Zero-loss control: potential_received = converted + active_pending + explicitly_closed; unaccounted_potential_orders must be zero.';
