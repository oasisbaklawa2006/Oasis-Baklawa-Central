-- PR #225: Catalogue AI-Studio Governance Completion Sprint.
-- Persists Catalogue Builder drafts with versioning, review workflow, and audit history.
-- Does NOT call any external AI/image/PDF/WhatsApp API. Does NOT mutate price or SKU.
-- Controlled live publish (if implemented in app code) is limited to description/image_url/shelf_life-storage,
-- and only ever touches public.products via the same governed client already used by AdminProducts.
...
COMMENT ON TABLE public.catalogue_ai_studio_drafts IS
  'PR #225: Catalogue Builder AI-Studio draft persistence, versioning, and review workflow. No external AI call, no price/SKU mutation.';
...
-- =============================================================================
-- 5. RLS — internal staff only, same helper used by catalogue_product_mappings.
